...
class CachedKernelRepository(KernelRepository):
    """
    Decorator: wraps any KernelRepository with LRU in-memory caching.
    ...
    def __init__(
        ...
        self._cache: OrderedDict[tuple[str, str | None, int], tuple[float, list[Kernel]]] = (
            OrderedDict()
        )
    ...
    async def promote(self, kernel_id: UUID) -> None:
        await self._wrapped.promote(kernel_id)

    async def deprecate(self, kernel_id: UUID, reason: str) -> None:
        await self._wrapped.deprecate(kernel_id, reason)
