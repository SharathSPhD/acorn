...
if [[ "${1:-}" =~ ^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$ ]]; then
    ...
    RESP=$(curl -sf -X POST "$ACORN_API/api/problems" \
...
WORKTREE_PATH="${HOME}/acorn-workspaces/problem-${PROBLEM_UUID}"

echo "[new-problem] UUID    : $PROBLEM_UUID"
echo "[new-problem] Network : $ACORN_NETWORK"

if [ -d "$WORKTREE_PATH" ]; then
    echo "[new-problem] Worktree already exists at $WORKTREE_PATH"
else
    git -C "${ACORN_ROOT}" worktree add -b "$BRANCH" "$WORKTREE_PATH" main 2>/dev/null || \
    git -C "${ACORN_ROOT}" worktree add "$WORKTREE_PATH" "$BRANCH" 2>/dev/null || {
...
docker run -d \
    --name "acorn-harness-${PROBLEM_UUID}" \
    --network "${ACORN_NETWORK}" \
    ...
    -v "${ACORN_ROOT}/scripts/deny-patterns.txt:/workspace/scripts/deny-patterns.txt:ro" \
    acorn/harness:latest

echo "[new-problem] Workspace : $WORKTREE_PATH"
echo "[new-problem] Branch    : $BRANCH"
echo "[new-problem] Container : acorn-harness-${PROBLEM_UUID}"
echo "[new-problem] Logs      : docker logs -f acorn-harness-${PROBLEM_UUID}"
