...
# Receives: ACORN_TOOL_CMD, ACORN_AGENT_ID, ACORN_PROBLEM_UUID via environment
...
if [ -f "$DENY_FILE" ]; then
    while IFS= read -r pattern; do
        ...
        [[ "$pattern" == ACORN:* ]] && continue  # ACORN: rules are Layer 2
        if echo "$CMD" | grep -qiE "$pattern"; then
            echo "[ACORN-PROXY] BLOCKED by deny list: $pattern" >&2
...
redis-cli -u "$REDIS_URL" LTRIM "acorn:session:${AGENT}:cmd_history" 0 49 >/dev/null 2>&1 || true
