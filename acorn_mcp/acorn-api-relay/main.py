"""
acorn-api-relay: Routes Claude Code API calls to Ollama or Claude API.
Implements the ACORN Harness Relay layer (HARNESS RELAY in architecture).
...
app = FastAPI(title="ACORN API Relay", version="0.1.0")

OLLAMA_BASE_URL = os.environ.get("OLLAMA_BASE_URL", "http://acorn-ollama:11434")
...
def _log_escalation(problem_uuid: str | None) -> None:
    ...
    try:
        ...
        if problem_uuid:
            r.hincrby(f"acorn:telemetry:problem:{problem_uuid}", "escalations", 1)
        r.close()
    except Exception:
...
def _log_call() -> None:
    ...
    try:
        r = _redis_sync.from_url(REDIS_URL_PROXY, decode_responses=True, socket_timeout=1)
        r.incr("acorn:telemetry:total_calls")
        r.close()
    except Exception:
        pass
...
async def messages(request: Request) -> Response:
    ...
    if should_escalate and ANTHROPIC_API_KEY_REAL:
        ...
        _log_escalation(request.headers.get("x-acorn-problem-uuid"))
        return Response(
...
async def proxy(request: Request, path: str) -> Response:
    ...
    if should_escalate and ANTHROPIC_API_KEY_REAL:
        ...
        problem_uuid = headers.get("x-acorn-problem-uuid")
