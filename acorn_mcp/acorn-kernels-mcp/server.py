#!/usr/bin/env python3
"""acorn-kernels-mcp: MCP server for kernel grove lookup and promotion."""
...
server = Server("acorn-kernels-mcp")
DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://acorn:acorn@acorn-postgres:5432/acorn")
KERNEL_PROMO_THRESHOLD = int(os.environ.get("ACORN_KERNEL_PROMO_THRESHOLD", "2"))
    ...
    try:
        if name == "find_skills":
            ...
            row = await conn.fetchrow(
                "SELECT verified_on_problems FROM kernels WHERE id = $1", kernel_id
            )
            if row is None:
                return [
                    types.TextContent(
                        type="text", text=json.dumps({"error": "kernel not found"})
                    )
                ]
            verified = row["verified_on_problems"] or []
            if len(verified) < KERNEL_PROMO_THRESHOLD:
                return [types.TextContent(type="text", text=json.dumps({
                    "status": "threshold_not_met",
                    "verified_count": len(verified),
                    "required": KERNEL_PROMO_THRESHOLD,
                }))]
            await conn.execute(
                "UPDATE kernels SET status='permanent', updated_at=NOW() WHERE id=$1", kernel_id
            )
